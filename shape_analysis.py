...
def analyse_seg(config):
    ...
    time_point = config['time_point']
    seg_file = os.path.join(config['seg_folder'], config['embryo_name'], config['embryo_name']+"_"+str(time_point).zfill(3)+'_segCell.nii.gz')
    # print(time_point, seg_file)
    seg = nib.load(seg_file).get_fdata()
    all_labels = np.unique(seg).tolist()
    all_labels.remove(0)

    nucleus_loc_to_save = pd.DataFrame.from_dict({"label": ["{}_{}".format(time_point, 1+all_labels.index(one_label)) for one_label in all_labels],
                                                  "surface":[0 for one_label in all_labels],
                                                  "volume":[0 for one_label in all_labels]})
    # seg = nib.load(seg_file).get_fdata()
    ace_shape = config['image_size'].copy()
    config["res"] = ace_shape[-1] / seg.shape[-1] * config["xy_resolution"]
    ...
    for one_label in all_labels:
        ...
        nucleus_loc_to_save.loc[nucleus_loc_to_save.label == "{}_{}".format(time_point, 1+all_labels.index(one_label)), "surface"] = surface_area
        nucleus_loc_to_save.loc[nucleus_loc_to_save.label == "{}_{}".format(time_point, 1+all_labels.index(one_label)), "volume"] = volume
    ...
    # Time_label    ***             ****
    #  add connections between SegCell (edge and edge weight)
    contact_pairs, contact_ares = add_relation_nolineage(seg)  # TODO: save output as CSV
    temp_dict = {'label':["{}_{}".format(time_point, 1+all_labels.index(one_label)) for one_label in all_labels]}
    for i in all_labels:
        temp_dict[1+all_labels.index(i)]=[0 for j in all_labels]
    contact_to_save = pd.DataFrame.from_dict(temp_dict)
    for i in range(len(contact_pairs)):
        contact_to_save.loc[contact_to_save.label == "{}_{}".format(time_point, 1+all_labels.index(contact_pairs[i][0])), all_labels.index(contact_pairs[i][1])+1] = contact_ares[i]



    save_name = os.path.join(config["project_folder"], 'Contact', config['embryo_name'],
                             str(config['time_point']).zfill(3) + '.csv')
    check_folder_exist(save_name)
    contact_to_save.to_csv(save_name, index=False)
    # Save format
    # contact_pairs:    (label1, label2) ...;  contact_areas:   are

def add_relation_nolineage(division_seg):
    '''
    Add relationship information between SegCell. (contact surface area)
    :param point_graph: point graph of SegCell
    :param division_seg: cell segmentations
